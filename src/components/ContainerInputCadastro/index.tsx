...
function ContainerInputCadastro() {
  ...
  const verificaExisteNomeUsuarioCadastrado = async () => {
    if (nomeUsuario) {
      try {
        const response = await fetch(`http://localhost:8080/api/livros/verificar-nome-usuario?nomeUsuario=${nomeUsuario}`);
  ...
  async function verificaExisteEmailCadastrado() {
    if (email) {
        try {
            const response = await fetch(`http://localhost:8080/api/livros/verificar-email-usuario?email=${email}`);
            const exists = await response.json();
            return exists;
        } catch (error) {
  ...
  const validate =  async () => {
    ...
    if (senha !== confirmacaoSenha) 
        errors.push("As senhas informadas não coincidem. Por favor, verifique e insira as senhas novamente");
    if (usuarioExistente)
        errors.push("O nome de usuário informado já está em uso. Por favor, escolha um nome de usuário diferente");
    if (emailExistente)
        errors.push("O E-mail informado já está cadastrado");
  ...
  const handleSubmit = async (e) => {
    ...
    if (isValid) {
      ...
      try {
        const response = await fetch('http://localhost:8080/api/livros/cadastrar/usuario', {
          method: 'POST',
          headers: {
  ...
  return (
    <form onSubmit={handleSubmit}>
        ...
        {successMessage.length > 0 && <SuccessMessage messages={successMessage}/>} 
        <ContainerInputWrap>
          <input
        ...
        <ContainerInputWrap>
          ...
          />
          <span className="focus-input" data-placeholder="Confirmação Senha"></span>
        </ContainerInputWrap>

        <ContainerLoginForm>
          <button
            className="login-form-btn" type="submit"
          >
            Cadastrar-se
          </button>
        </ContainerLoginForm>
